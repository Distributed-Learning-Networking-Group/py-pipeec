...
from pypipeec import checkpoint, config
...
def checkpoint_task(
    ...
    checkpoint_context.save_cuda(model, optimizer, rng)
    checkpoint_context.check_point()
    checkpoint_context.wait()
...
def run_epoch(
    ...
    for inputs, targets in pbar:
        ...
        if 0 < NITERS <= num_iters:
            sys.exit(0)

        if num_iters % CHECK_CYCLE == 0:
            fut = executor.submit(
                checkpoint_task,
                checkpoint_context,
                model.model(),
                optimizer,
                checkpoint_context.gather_rng_states()
            )
        ...
        if targets is not None:
            targets = distributed.cuda.to(
                device, targets, non_blocking=False)
        outputs = model.forward(inputs, checkpoint_context.suspend)
        # outputs = model.forward(inputs)
        if last_stage:
            losses = distributed.loss(outputs, targets, loss_function)
            for loss in losses:
        ...
        checkpoint_context.resume()
        model.backward(losses, checkpoint_context.suspend)
...
def network_configs(
    ...
    addrs = {
        0: "192.168.124.107:19737",
        1: "192.168.124.101:19737"
    }
    # addrs = {
    #     0: "localhost:17382",
    #     1: "localhost:17387",
...
def cli(
    ...
    with distributed.run(workers[rank], chunks, torch.device(device)):
        ...
        if not dry_run:
            checkpoint_context.load_cuda(model.model(), optimizer)
